import type { Post } from "./types";

export const posts: Post[] = [
  {
    slug: "tucker-wetmore-contract-battle-management-drama",
    title: "Tucker Wetmore Hit With Explosive Contract Battle Amid Management Drama",
    subheadline:
      "Country star faces legal showdown after alleged breach and secret talks shake Nashville.",
    excerpt:
      "Country star Tucker Wetmore is suddenly facing major legal drama after Back Blocks Music reportedly accused him of breaching his management contract.",
    author: "Jane Lytvynenko",
    date: "2026-05-16T10:00:00.000Z",
    readTime: "6 min read",
    category: "Music",
    tags: ["Tucker Wetmore", "Music", "Legal"],
    image: {
      src: "https://ext.same-assets.com/616080166/3217493409.webp",
      alt: "Tucker Wetmore performing on stage under red lights",
      width: 1600,
      height: 900,
    },
    content: `<p>Country music's rising star Tucker Wetmore finds himself at the center of an escalating legal battle that threatens to derail one of Nashville's most promising careers.</p>
<p>According to court documents obtained exclusively by Fame Ticker, Back Blocks Music has filed a breach-of-contract lawsuit against Wetmore, alleging that the singer attempted to terminate his management agreement prematurely and without proper legal grounds.</p>
<p>The lawsuit, filed in Davidson County Circuit Court on May 14, claims Wetmore signed a multi-year management deal in 2024 that included standard industry provisions for commission on all entertainment earnings. Sources close to the situation say the dispute centers on disagreements over creative direction and financial transparency.</p>
<p>"This is a clear-cut case of a contractual obligation," a representative for Back Blocks Music told Fame Ticker. "We supported Tucker from the very beginning, and we intend to enforce our agreement."</p>
<p>Wetmore's legal team has not yet filed a formal response but released a statement suggesting the contract contained "unconscionable terms" that no emerging artist should be bound by.</p>
<p>Industry insiders say the case could have broader implications for how management contracts are structured in Nashville, particularly for young artists who sign deals before achieving mainstream success.</p>`,
    isBreaking: true,
    featured: true,
    trending: true,
    popular: true,
  },
  {
    slug: "man-pleads-guilty-to-stealing-beyonce-unreleased-music",
    title:
      "Man Pleads Guilty to Stealing Beyoncé's Unreleased Music During Cowboy Carter Tour",
    excerpt:
      "Hard drives containing unfinished recordings were taken from a rental vehicle in Atlanta. The culprit now faces prison time.",
    author: "Marcus Webb",
    date: "2026-05-13T12:30:00.000Z",
    readTime: "4 min read",
    category: "Music",
    tags: ["Beyoncé", "Music", "Legal"],
    image: {
      src: "https://ext.same-assets.com/616080166/3925710785.webp",
      alt: "Beyoncé in concert wearing a shimmering outfit",
      width: 1600,
      height: 900,
    },
    content: `<p>A 34-year-old man has pleaded guilty to stealing unreleased music from Beyoncé's Cowboy Carter tour production.</p>
<p>Federal prosecutors say the suspect removed hard drives containing unfinished recordings from a rental vehicle used by the production team in Atlanta. He now faces up to five years in federal prison.</p>
<p>The incident sparked a multistate investigation after unreleased music briefly surfaced online. Authorities say the accused coordinated a targeted theft during a tour stop.</p>`,
    trending: true,
    popular: true,
  },
  {
    slug: "taylor-swift-travis-kelce-engaged",
    title: "Taylor Swift and Travis Kelce Are Officially Engaged — She Wore the Ring to Prove It",
    excerpt:
      "Swifties spotted a stunning ring during the awards show, and sources confirm the engagement is real.",
    author: "Sarah Chen",
    date: "2026-05-12T14:00:00.000Z",
    readTime: "5 min read",
    category: "Relationships",
    tags: ["Taylor Swift", "Travis Kelce", "Relationships"],
    image: {
      src: "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?q=80&w=1600&auto=format&fit=crop",
      alt: "Couple holding hands with a ring in focus",
    },
    content: `<p>It's official — Taylor Swift and Travis Kelce are engaged. Eagle-eyed Swifties first spotted the ring during a recent awards ceremony, and multiple sources have now confirmed the news.</p>
<p>Rumors turned into fireworks when an eagle-eyed fan close-up revealed the ring's details. The couple has been at the center of engagement speculation for months, and this appears to be the confirmation fans have been waiting for.</p>`,
    trending: true,
  },
  {
    slug: "doechii-met-gala-barefoot-plum-marc-jacobs",
    title:
      "Doechii Hit the Met Gala Red Carpet Barefoot in a Plum Marc Jacobs Look — Internet Goes Wild",
    excerpt:
      "Unexpected, irreverent, and very Doechii, the look may be the night's most viral moment.",
    author: "Mia Torres",
    date: "2026-05-06T23:20:00.000Z",
    readTime: "3 min read",
    category: "Fashion",
    tags: ["Doechii", "Met Gala", "Fashion"],
    image: {
      src: "https://images.unsplash.com/photo-1581368129680-4ed6b2ee2050?q=80&w=1600&auto=format&fit=crop",
      alt: "Purple gown swirling on a lit staircase",
    },
    content: `<p>Leave it to Doechii to break every rule on fashion's biggest night. The rapper arrived at the 2026 Met Gala barefoot, wearing a sweeping plum Marc Jacobs gown that instantly became the most talked-about look of the evening.</p>
<p>The barefoot moment divided fashion Twitter and delighted the red carpet's chaos enthusiasts. Critics called it irreverent genius; traditionalists called it disrespectful. Either way, Doechii owned the night.</p>`,
    trending: true,
  },
  {
    slug: "naked-dress-back-met-gala-debated-trend",
    title:
      "The Naked Dress Is Back — And the 2026 Met Gala Made It Fashion's Most Debated Trend",
    excerpt:
      "From Gigi Hadid to Simone Ashley, the sheer look dominated this year's carpet. Not everyone is happy about it.",
    author: "Mia Torres",
    date: "2026-05-05T11:20:00.000Z",
    readTime: "7 min read",
    category: "Fashion",
    tags: ["Met Gala", "Fashion", "Red Carpet"],
    image: {
      src: "https://ext.same-assets.com/616080166/3223465574.webp",
      alt: "A sheer couture dress on the red carpet",
      width: 1600,
      height: 900,
    },
    content: `<p>The naked dress made its triumphant — or controversial — return at the 2026 Met Gala, with multiple A-listers opting for sheer, skin-baring designs that left little to the imagination.</p>
<p>Designers embraced translucency with fearless precision, pushing boundaries while flirting with controversy. Gigi Hadid, Simone Ashley, and several others led the charge in barely-there looks that dominated the post-Gala conversation.</p>
<p>Critics argue the trend has become predictable, while supporters say it's a celebration of body confidence and artistic expression. Either way, the naked dress isn't going anywhere.</p>`,
    trending: true,
  },
  {
    slug: "margot-robbie-met-gala-gown-761-hours",
    title: "Margot Robbie's Met Gala Gown Took 761 Hours to Craft — And It Shows",
    excerpt:
      "The golden Chanel creation, with its sweeping ruffled train, is already being called the dress of the decade.",
    author: "Lena Park",
    date: "2026-05-05T09:00:00.000Z",
    readTime: "5 min read",
    category: "Fashion",
    tags: ["Margot Robbie", "Met Gala", "Fashion", "Red Carpet"],
    image: {
      src: "https://ext.same-assets.com/616080166/3570463308.webp",
      alt: "Margot Robbie posing at the Met Gala in a gold gown",
      width: 1600,
      height: 900,
    },
    content: `<p>Margot Robbie stunned at the 2026 Met Gala in a golden Chanel gown that reportedly took 761 hours to create. The sweeping ruffled train is already being called the dress of the decade by fashion critics worldwide.</p>
<p>The intricate detailing, hand-finished appliqués, and architectural structure demanded nearly a month of studio time. Every ruffle was individually placed, every bead hand-sewn by Chanel's most experienced artisans.</p>`,
    trending: true,
    popular: true,
  },
  {
    slug: "beyonce-returns-met-gala-skeleton-gown-blue-ivy",
    title:
      "Beyoncé Returns to Met Gala After 10 Years in a Skeleton Gown — and Brings Blue Ivy",
    excerpt:
      "Fashion's biggest night got its biggest moment in years when Queen B walked the steps with her daughter by her side.",
    author: "Lena Park",
    date: "2026-05-04T18:00:00.000Z",
    readTime: "6 min read",
    category: "Fashion",
    tags: ["Beyoncé", "Blue Ivy", "Met Gala", "Fashion"],
    image: {
      src: "https://images.unsplash.com/photo-1509631179647-0177331693ae?q=80&w=1600&auto=format&fit=crop",
      alt: "Elegant couture gown on red carpet stairs",
      width: 1600,
      height: 1000,
    },
    content: `<p>Beyoncé made a stunning return to the Met Gala after a decade-long absence, arriving in a jaw-dropping skeleton-inspired gown with daughter Blue Ivy by her side.</p>
<p>After a decade away, Beyoncé's return dominated timelines and set the internet ablaze. The mother-daughter moment was the emotional core of the evening, with Blue Ivy matching her mother's elegance at every step.</p>`,
    popular: true,
  },
  {
    slug: "rihanna-maison-margiela-met-gala-vision",
    title:
      "Rihanna Wore Shimmering Maison Margiela Couture to the Met Gala and Looked Like a Vision",
    excerpt:
      "The Fenty mogul delivered another masterclass in dressing for maximum impact.",
    author: "Mia Torres",
    date: "2026-05-04T19:15:00.000Z",
    readTime: "4 min read",
    category: "Fashion",
    tags: ["Rihanna", "Met Gala", "Fashion"],
    image: {
      src: "https://images.unsplash.com/photo-1598488035139-bdbb2235a20c?q=80&w=1600&auto=format&fit=crop",
      alt: "Rihanna in couture with shimmering textures",
    },
    content: `<p>Rihanna once again proved why she's fashion's most fearless icon, arriving at the 2026 Met Gala in shimmering Maison Margiela couture that left the crowd speechless.</p>
<p>Dramatic structure met cinematic sparkle — Rihanna remains the undisputed moment-maker. Every appearance she makes at the Met Gala rewrites the rules of what red carpet fashion can be.</p>`,
  },
  {
    slug: "zendaya-dune-sequel-style-tour",
    title: "Zendaya's Dune II Style Tour Is a Masterclass in Sci‑Fi Glamour",
    excerpt:
      "From armor-inspired silhouettes to liquid metallics, Zendaya is rewriting the red carpet rulebook.",
    author: "Sarah Chen",
    date: "2026-04-22T09:30:00.000Z",
    readTime: "6 min read",
    category: "Movies",
    tags: ["Zendaya", "Dune", "Movies", "Fashion"],
    image: {
      src: "https://images.unsplash.com/photo-1516570161787-2fd917215a3d?q=80&w=1600&auto=format&fit=crop",
      alt: "Avant-garde metallic dress on a blue carpet",
    },
    content: `<p>Zendaya's press tour for the Dune sequel has become a fashion event in its own right, with the actress delivering look after look of sci-fi inspired glamour.</p>
<p>The press tour doubles as a runway where references to sci‑fi classics blend with modern couture. From armor-inspired silhouettes to liquid metallics, every appearance pushes the boundaries of what a promotional tour can look like.</p>`,
  },
  {
    slug: "summer-blockbusters-2026-what-to-watch",
    title: "Summer Blockbusters 2026: 9 Movies Everyone Will Be Talking About",
    excerpt:
      "Sequels, reboots, and original surprises line up for a competitive, meme-ready season.",
    author: "David Kim",
    date: "2026-04-30T10:00:00.000Z",
    readTime: "9 min read",
    category: "Movies",
    tags: ["Movies", "Summer 2026", "Blockbusters"],
    image: {
      src: "https://images.unsplash.com/photo-1524985069026-dd778a71c7b4?q=80&w=1600&auto=format&fit=crop",
      alt: "Cinema screen with dramatic lighting",
    },
    content: `<p>Summer 2026 is shaping up to be one of the most competitive movie seasons in years, with sequels, reboots, and surprise originals all vying for box office dominance.</p>
<p>Studios are betting big on crowd-pleasing spectacles with a few risky swings. From long-awaited franchise continuations to bold original concepts, here are the nine films everyone will be talking about.</p>`,
  },
  {
    slug: "drake-surprise-mixtape-leaks-teasers",
    title: "Drake Drops Surprise Mixtape After Weeks of Cryptic Teasers",
    excerpt:
      "The Toronto rapper's midnight drop has fans decoding bars and hidden meanings across social media.",
    author: "Marcus Webb",
    date: "2026-04-10T00:05:00.000Z",
    readTime: "3 min read",
    category: "Music",
    tags: ["Drake", "Music"],
    image: {
      src: "https://images.unsplash.com/photo-1511379938547-c1f69419868d?q=80&w=1600&auto=format&fit=crop",
      alt: "Rapper performing on stage with purple lights",
    },
    content: `<p>Drake surprised fans with a midnight mixtape drop after weeks of cryptic social media posts that had the internet buzzing with speculation.</p>
<p>The project features a range of collaborators and nods to early 2010s sounds. Within hours, fans had already begun decoding bars, references, and hidden meanings across social media.</p>`,
  },
  {
    slug: "celebrity-couples-2026-whos-in-whos-out",
    title: "Celebrity Couples 2026: Who's In, Who's Out, Who's Complicated",
    excerpt:
      "From secret weddings to mutual unfollows, here are the relationship status updates you missed.",
    author: "Sarah Chen",
    date: "2026-03-29T08:00:00.000Z",
    readTime: "8 min read",
    category: "Relationships",
    tags: ["Relationships", "Celebrities"],
    image: {
      src: "https://images.unsplash.com/photo-1511512578047-dfb367046420?q=80&w=1600&auto=format&fit=crop",
      alt: "Polaroids of celebrity couples on a corkboard",
    },
    content: `<p>2026 has already delivered some jaw-dropping relationship updates from Hollywood's most watched couples.</p>
<p>A whirlwind month in Hollywood left the internet dizzy. From secret weddings that nobody saw coming to mutual unfollows that sparked breakup rumors, here's your complete guide to who's together, who's not, and who's making it complicated.</p>`,
  },
  {
    slug: "reality-tv-villain-redemption-arc-2026",
    title: "Reality TV's Favorite Villain Just Got a Redemption Arc — Here's How",
    excerpt:
      "A viral apology, a teary reunion, and a surprise casting twist flipped the narrative overnight.",
    author: "Jake Morrison",
    date: "2026-05-01T17:00:00.000Z",
    readTime: "4 min read",
    category: "Reality TV",
    tags: ["Reality TV"],
    image: {
      src: "https://images.unsplash.com/photo-1521133573892-e44906baee46?q=80&w=1600&auto=format&fit=crop",
      alt: "TV production stage with bright lights",
    },
    content: `<p>In one of the most dramatic turnarounds in reality television history, the season's most-hated villain has undergone a complete redemption arc.</p>
<p>Public perception changed almost instantly after a candid long-form interview appeared online. A viral apology, a teary reunion with a former rival, and a surprise casting twist flipped the narrative overnight. Fans who swore they'd never forgive are now leading the redemption campaign.</p>`,
    trending: true,
  },
];
